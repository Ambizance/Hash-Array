...
ListSmaller = Array.new
ListFiveCharacter = Array.new



#Combien y a-t-il de handle dans cette array ?
def count
puts "Il y a #{List.length} handles dans le tableau"
end
...
(List.length).times do |i|
    ListSmaller[i] = List[i].length
    end 
puts List[ListSmaller.index((ListSmaller.min))]
...
def five_characters
 ...
 List.length.times do |i|
    ...
    if List[i].length == 6
        ListFiveCharacter.push"#{List[i]}"
    end
end
    puts ListFiveCharacter
end

#Combien commencent par une majuscule (première lettre juste après le @) ?

def majuscule
nbMotsMaj = List.count { |handle| /[[:upper:]]/.match(handle[1])} 
puts "Il y a #{nbMotsMaj} qui commencent par une majuscule dans la liste !"
end
...
def repartition
    for i in (List.min_by(&:length).length)..(List.max_by(&:length).length) do
        if ((List.count {|word| word.length == i}) != 0)
        puts "Il y a #{List.count {|word| word.length == i }} handles avec #{i} caractères"
        end 
    end
end
